...
let networkingV1Api: NetworkingV1Api;
...
async function refreshKubernetesToken() {
  try {
    ...
    logger.info(
      'Successfully refreshed Kubernetes access token and recreated API clients'
    );
  } catch (error) {
...
app.post('/start', async (req, res) => {
  ...
  try {
    ...
    logger.info(
      'Checking for and cleaning up any existing resources before start.',
      {replId}
    );
